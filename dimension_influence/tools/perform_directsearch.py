...
from problems.ProblemEigh import (
    ProblemEigh,
)

from problems.ProblemLinearSubspace import (
...
def directsearch(
    ...
    while k < itmax and alpha > eps and evaluation_number < budget:
        ...
        while i < len(pss_at_x) and success == False:
            ...
            evaluation_number += 1
            if (
                f_poll <= f_value - c * alpha**2 * problem.anorm(p) ** 2
        ...
        if returnforeuclsimplex == 1:
            if (
                evaluation_number <= rbudget or k == 1
    ...
    if (
        stopping_criterion == "full iterations used"
        or stopping_criterion == "low step size"
    ):
    ...
    for imdim, mdim in enumerate(mdims):
        ...
        for icodim, codim in enumerate(codims):
                ...
                for projection in [0, 1]:
                    for rotation in [0, 1]:
                        for psstype in [1, 2, 3]:
                            ...
                            budget = N * (
                                problem.mdim * (euclideansimplex == 0)
                                + problem.adim * (euclideansimplex == 1)
                                + 1
                            )
                            ...
                            dsresult = directsearch(
                                ...
                                psstype=psstype,
                                renormalize_tangent_vec=True,
                                returnforeuclsimplex=euclideansimplex,
                                eps=0,
                            )
                            ...
                            if euclideansimplex == 1:
                                ...
                                dsresults[imdim][icodim][k][key1] = dsresult[
                                    1
                                ]  # the euclidean simplex computations
                                key0 = ds_key(0, projection, rotation, psstype)
                                dsresults[imdim][icodim][k][key0] = dsresult[
                                    0
                                ]  # the riemannian simplex (truncation)
            ...
            if printingprogression:
                ...
                print(
                    "newwdim : {:} out of {:}, cumulated_time = {:<2}:{:<2}:{:<2}, newtime = {:.2f}s".format(
                        count,
...
def saveperform_ds(
    ...
    """
    nbr = (
        "exp" + str(expnumber) + "_maniftype" + str(maniftype) + "_obj" + str(obj) + "_"
    )
    print(
        "computing :",
        nbr,

...
def dataprofile(
    vdsresults, vproblems, tau=1e-2, euclideansimplex=0, lbdfoeuclideansimplex=1, N=100
):
    """Arguments :
    ...
    vlbdfo = [
        [
            [[None for k in range(nbinstances)] for icodim in range(ncodims)]
            for imdim in range(nmdims)
        ]
        for nb_problem in range(nb_problems)
    ]  # lower bound proxy for each problem instance
    vfalpha = np.zeros((nb_problems, 2, 2, 3, ncodims, nmdims, nbinstances))
    dp_codim = np.zeros(
        (2, 2, 3, ncodims, N + 1)
    )  # one dataprofile per codim (fixed mdim)
    dp_mdim = np.zeros(
        (2, 2, 3, nmdims, N + 1)
    )  # one dataprofile per mdim (fixed codim)
    dp_tot = np.zeros(
        (2, 2, 3, N + 1)
    )  # one dataprofile for all dimensions of all problems
            ...
            for imdim, mdim in enumerate(mdims):
                ...
                for k in range(nbinstances):
                    ...
                    for (
                        ...
                        rotation,
                        psstype,
                        euclideansimplexexplo,
                    ) in itertools.product(
                        [0, 1], [0, 1], [1, 2, 3], range(lbdfoeuclideansimplex)
                    ):  # if lbdfoeuclideansimplex = 1, this explores the values given by a direct-search with budget scaled by the ambient dimension. Else only by the manifold dimension.
                        key = ds_key(
                            euclideansimplex=euclideansimplexexplo,
                        ...
                        min_ik = dsresult[key][
                            "last_fvalue"
                        ]  # assumes the DFO method yields only decreasing function values
                        if min_ik < lbdfo:
                            lbdfo = min_ik
                    ...
                    for projection, rotation, psstype in itertools.product(
                        ...
                        evaluations = np.cumsum(
                            dsresult[key]["v_ev_per_it"]
                        )  # cumulated evaluations at each iterate
    ...
    for projection, rotation, psstype, icodim in itertools.product(
        [0, 1], [0, 1], [1, 2, 3], np.arange(ncodims)
    ):
        ...
        for alpha in range(N + 1):
            ...
            ratio_solved = np.sum(
                vfalpha[:, projection, rotation, psstype - 1, icodim, imdim, :] <= alpha
    ...
    for projection, rotation, psstype, imdim in itertools.product(
        [0, 1], [0, 1], [1, 2, 3], np.arange(nmdims)
    ):
    ...
    for projection, rotation, psstype, imdim, icodim in itertools.product(
        [0, 1], [0, 1], [1, 2, 3], np.arange(nmdims), np.arange(ncodims)
    ):
        for alpha in range(N + 1):
            ratio_solved = np.sum(
...
def plotting_dp(
    ...
    mdims, codims, nbinstances, problems = (
        problem0["mdims"],
        problem0["codims"],
        problem0["nbinstances"],
        problem0["problems"],
    )
    ...
    if plotcodimsev:
            ...
            for rotation, projection, psstype in itertools.product(
                ...
                if icodim == len(codims) - 1:
                    ax[icodim].plot(
                        ...
                        linewidth=2,
                        label="PSS{:} ({:})".format(psstype, variant),
                    )
        ...
        if saving:
            ...
            figname = (
                ...
                + str(rotations)
                + "_es"
                + str(euclideansimplex)
                + "_eslbdfo"
                + str(lbdfoeuclideansimplex)
                + "_tau"
                + str(tau)
                + "_mdims"
                + str(mdims)
            ...
            plt.savefig(
                "tables_and_plots/dataprofiles/" + figname, bbox_inches="tight", dpi=300
            )
        plt.show()

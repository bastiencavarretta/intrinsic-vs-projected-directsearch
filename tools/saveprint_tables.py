...
def performancetable(
    ...
    mdims, codims, nbinstances = (
        loadedproblem["mdims"],
        loadedproblem["codims"],
        loadedproblem["nbinstances"],
    )
    ...
    for imdim, mdim in enumerate(mdims):
        for icodim, codim in enumerate(codims):
            ...
            for (euclideansimplex, rotation, psstype), grp in sub.groupby(
                level=["euclideansimplex", "rotation", "psstype"], sort=False
            ):

                euclideansimplex, rotation, psstype = (
                    int(euclideansimplex),
                    int(rotation),
                    int(psstype),
                )
                keyproj = ds_key(
                ...
                for k in range(nb_problems):
                    for l in range(nbinstances):
                        ...
                        gap = (
                            dsresult[keyproj]["last_fvalue"]
                            - dsresult[keynoproj]["last_fvalue"]
                        )
                        if gap > 0:
                            ratio += 1
                ratio = ratio / (nbinstances * nb_problems)
                table.loc[
                    (mdim, codim, euclideansimplex, rotation, psstype), "ratio"
                ] = ratio

    # printing and saving tables
    for rotation, euclideansimplex, psstype in itertools.product(
        rotations, euclideansimplexs, psstypes
    ):
        subtable = table.xs(
            (euclideansimplex, rotation, psstype),
            level=("euclideansimplex", "rotation", "psstype"),
        )

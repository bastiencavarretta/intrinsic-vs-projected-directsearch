...
def normalize(vectors, tol=10**-15):
    ...
    normalized_vectors[:, indices_zero] = np.zeros(
        (dim, len(indices_zero))
    )  # withdrawing small vectors
...
def compute_cm_HJB(M, tol=1e-15):
    ...
    for family in itertools.combinations(M.T, n):
        ...
        if np.abs(np.linalg.det(B)) >= tol:  # B must be a base
            ...
            gamma_B = 1 / np.sqrt(
                np.ones(n) @ lg.solve(G_B, np.ones(n))
            )  # more efficient : Cholesky and solve triangular systems
...
def CM_heatmap_spheredim2(
    ...
    n_samples=1000,
    min_theta=-0.1,
    max_theta=2 * np.pi + 0.1,
    min_phi=-0.1,
    max_phi=np.pi + 0.1,
    tol=10 ** (-10),
    return_intrinsic_cm=False,
    tol_HJB=1e-15,
):
    ...
    X = np.zeros(
        (N_theta, N_phi, n)
    )  # actual mesh on the sphere (will store the value of $x$ for every angles)
    ...
    for i in range(N_theta):
        for j in range(N_phi):
            ...
            e_theta = np.array([[-np.sin(theta), np.cos(theta), 0.0]]).reshape(-1, 1)
            e_phi = np.array(
                [
                    [
...
def CM_compute_anydim(
    m,
    psstype=1,
    n_samples=30,
    part_points=[],
    testing_intr_pss=False,
    tol_manifold=1e-12,
    tol_HJB=10 ** (-10),
    tol=10 ** (-10),
):
    ...
    nbr_part_points = len(
        part_points
    )  # index to slice the result for particular points.
    for part_point in part_points:
        if np.abs(np.linalg.norm(part_point, ord=2) - 1) > tol_manifold:
            raise ValueError(
                "object not in sphere. check if this is due to implementation or numerical error"
            )
    ...
    for k in tqdm(range(len(points))):
        ...
        pss_proj_base_tangentspace = base_tangentspace.T.dot(pss_proj)
        pss_intr_base_tangentspace = base_tangentspace.T.dot(
            pss_intr
        )  # reverse operation (to check the implementation).
        ...
        if (
            testing_intr_pss
        ):  # testing that I get the good computation for "trivial pss"
...
def compute_cm_with_dims(adims, psstype=1, n_samples=100, with_part_points=True):
    ...
    for k, n in enumerate(adims):
        ...
        X, part_points, CM, CM_part_points, cm_intr_th, cm_ambiant_th, CM_intr = (
            CM_compute_anydim(
                m,
                psstype=psstype,
                n_samples=n_samples,
